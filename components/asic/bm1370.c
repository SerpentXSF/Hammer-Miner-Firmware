...
static void _send_BM1370(uint8_t header, uint8_t * data, uint8_t data_len, bool debug)
{
    ...
    // send serial data
    if (SERIAL_send(0, buf, total_length, debug) == 0) {
...
static void _send_simple(uint8_t * data, uint8_t total_length)
{
    ...
    SERIAL_send(0, buf, total_length, BM1370_SERIALTX_DEBUG);
...
static void BM1370_send_chip_frequency(uint8_t chip_index, float target_freq)
{
    uint8_t fb_divider, refdiv, postdiv1, postdiv2;
    float frequency;
    ...
    // BC04 has 4 chips. Addresses split evenly: 256/4 = 64.
    uint8_t chip_addr = chip_index * 64;
    uint8_t freqbuf[6] = {chip_addr, 0x08, vdo_scale, fb_divider, refdiv, postdiv};

    _send_BM1370(TYPE_CMD | GROUP_SINGLE | CMD_WRITE, freqbuf, 6, BM1370_SERIALTX_DEBUG);

    ESP_LOGI(TAG, "Setting Chip %d (addr 0x%02X) Frequency to %g MHz (%g)", chip_index, chip_addr, target_freq, frequency);
}

#define EPSILON 0.0001f
#define STEP_SIZE 6.25 // MHz step size

static float current_chip_frequencies[4] = {50.0f, 50.0f, 50.0f, 50.0f};

static void do_bc04_individual_frequency_transition(uint16_t target_freqs[4])
{
    ESP_LOGI(TAG, "Ramping individual chip frequencies concurrently: Chip0=%d, Chip1=%d, Chip2=%d, Chip3=%d",
             target_freqs[0], target_freqs[1], target_freqs[2], target_freqs[3]);

    bool all_done = false;
    while (!all_done) {
        all_done = true;
        for (int i = 0; i < 4; i++) {
            float target = target_freqs[i];
            float current = current_chip_frequencies[i];
            
            if (fabs(current - target) > EPSILON) {
                float next_freq;
                if (fabs(target - current) < STEP_SIZE) {
                    next_freq = target;
                } else {
                    int signum = (target > current) ? 1 : -1;
                    next_freq = current + (signum * STEP_SIZE);
                }
                
                BM1370_send_chip_frequency(i, next_freq);
                current_chip_frequencies[i] = next_freq;
                all_done = false;
                vTaskDelay(10 / portTICK_PERIOD_MS); // Add a small 10ms delay between chip writes to prevent serial collisions
            }
        }
    ...
    ESP_LOGI(TAG, "Successfully transitioned all chips to target frequencies.");
}
#endif
...
#ifdef CONFIG_BC04_INDIVIDUAL_FREQ
    char *model = nvs_config_get_string(NVS_CONFIG_DEVICE_MODEL, "default");
    bool is_bc04 = (strcmp(model, "BC04") == 0);
    free(model);

    if (is_bc04) {
        uint16_t target_freqs[4];
        uint16_t boot_mode = nvs_config_get_u16(NVS_CONFIG_BOOT_MODE, 0);

        if (boot_mode == USER_CUSTOMIZED_MODE) {
            target_freqs[0] = nvs_config_get_u16("asicfreq_c0", (uint16_t)frequency);
            target_freqs[1] = nvs_config_get_u16("asicfreq_c1", (uint16_t)frequency);
            target_freqs[2] = nvs_config_get_u16("asicfreq_c2", (uint16_t)frequency);
            target_freqs[3] = nvs_config_get_u16("asicfreq_c3", (uint16_t)frequency);
            do_bc04_individual_frequency_transition(target_freqs);
        } else {
...
task_result * BM1370_process_work(void * pvParameters)
{
    ...
    if (receive_work_bm((uint8_t *)&asic_result, sizeof(asic_result)) == ESP_FAIL) {
        return NULL;
    }
    ...
    if (!asic_result.is_job_response) {
        ...
        GlobalState * GLOBAL_STATE = (GlobalState *) pvParameters;
        int chip_counter = GLOBAL_STATE->asic_count[0];
        if (chip_counter > 1) {
            uint8_t address_interval = (uint8_t) (256 / chip_counter);
            uint8_t chip_addr = asic_result.cmd.asic_nr << 1;
            result.asic_nr = chip_addr / address_interval;
        } else {
            result.asic_nr = 0;
        }
    ...
    if (GLOBAL_STATE->valid_jobs[0][job_id] == 0) {
        ESP_LOGW(TAG, "Invalid job nonce found, 0x%02X", job_id);
        return NULL;
    }

    uint32_t rolled_version = GLOBAL_STATE->ASIC_TASK_MODULE[0].active_jobs[job_id]->version | version_bits;
    ...
    for (int i = 0; i < chip_counter; i++) {
        ...
        for (int reg = 0; reg < size; reg++) {
            if (REGISTER_MAP[reg] != REGISTER_INVALID) {
                _send_BM1370((TYPE_CMD | GROUP_SINGLE | CMD_READ), (uint8_t[]){chip_addr, reg}, 2, false);
