...
{
    return bc_i2c_add_device(slave_addr, tmp75_dev_handles+temperature_sensor_index, TAG);
}
...
esp_err_t TMP75_installed(int temperature_sensor_index)
{
    ...
    result = bc_i2c_register_read(tmp75_dev_handles[temperature_sensor_index], TMP75_CONFIG_REG, data, 2);
