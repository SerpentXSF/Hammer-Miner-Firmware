...
static void event_handler(void * arg, esp_event_base_t event_base, int32_t event_id, void * event_data)
{
    if (event_base == WIFI_EVENT)
    {
        ...
        if (event_id == WIFI_EVENT_STA_START) {
            esp_wifi_connect();
            network_set_wifi_status(WIFI_CONNECTING, 0, 0);
        } else if (event_id == WIFI_EVENT_STA_DISCONNECTED) {
        	network_set_wifi_status(WIFI_DISCONNECTED, 0, 0);
            ...
            if(s_retry_num >= 5 && WIFI_REASON_NO_AP_FOUND == event->reason){
                esp_wifi_disconnect();
                ESP_LOGI(TAG, "Stop retrying to connect to %s", event->ssid);
                xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
                network_set_wifi_status(WIFI_DISCONNECTED, s_retry_num, 0);
                return;
            }
            if(s_retry_num > 12)
            {
            ...
            network_set_wifi_status(WIFI_RETRYING, s_retry_num, event->reason);
