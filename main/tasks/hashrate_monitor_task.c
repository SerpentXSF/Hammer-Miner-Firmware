...
void hashrate_monitor_task(void *pvParameters)
{
    ...
    int asic_count = GLOBAL_STATE->asic_count[chain_num];
    ...
    bool supports_register_reading = (GLOBAL_STATE->device_model == DEVICE_BC04);
    ...
    while (1) {
        ...
        if (hashrate == 0.0) {
            ...
            if (HASHRATE_MONITOR_MODULE->hashrate == 0.0f) {
                HASHRATE_MONITOR_MODULE->hashrate = GLOBAL_STATE->asic_freqency*2040*2040/1000;
            }
        ...
        print_count++;
        if (print_count >= 6) { // 6 * 5 seconds = 30 seconds
            print_count = 0;
            #ifdef ENABLE_CHIP_HASH_PRINT
            ESP_LOGI(TAG, "ASIC individual hashrates (GH/s):");
            for (int i = 0; i < asic_count; i++) {
                float hw_rate = HASHRATE_MONITOR_MODULE->total_measurement[i].hashrate;
                #ifdef STATISTIC_SYSTEM_FEATURE
                double sw_rate = 0.0;
                statistic_get_chip_hashrate(&(GLOBAL_STATE->STATISTIC_MODULE), chain_num, i, &sw_rate);
                ESP_LOGI(TAG, "  Chip %d: Phys %.2f | Sw %.2f", i, hw_rate, sw_rate);
                #else
                ESP_LOGI(TAG, "  Chip %d: Phys %.2f", i, hw_rate);
                #endif
            }
            #endif
        }
...
void hashrate_monitor_register_read(void *pvParameters, register_type_t register_type, uint8_t asic_nr, uint32_t value)
{
    ...
    int asic_count = GLOBAL_STATE->asic_count[0];
    ...
    if (GLOBAL_STATE->asic_freqency != frequency_value) {
        clear_measurements(HASHRATE_MONITOR_MODULE, asic_count);
        frequency_value = GLOBAL_STATE->asic_freqency;
    }
